...
export function complianceRoutes(app: FastifyInstance) {
  ...
  app.get('/api/popia/subject', async (req, reply) => {
    ...
    const contacts = await sql`select id, name, type, phone, email, created_at, merged_into from contacts
      where name ilike ${like} or phone ilike ${like} or email ilike ${like} order by id`;
    ...
    await audit(sql, { actor: req.user.id, action: 'popia.subject_export', entity: 'report', id: q, data: { tickets: tickets.length, bleeds: bleeds.length, contacts: contacts.length }, ip: req.ip });
    ...
    return { generated_at: new Date().toISOString(), query: q, contacts, tickets, bleeds, access_log: access };
  });
}
